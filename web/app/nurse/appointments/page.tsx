...
import { Calendar, ChevronRight, FileText, Loader, ArrowLeft } from 'lucide-react';
    ...
    <div className="px-6 py-8 pb-24 md:pb-8 max-w-3xl mx-auto w-full min-h-screen flex flex-col bg-[#FBF9F6]">
      <header className="mb-8 flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500 font-light mt-1">Riwayat jadwal perawatan Anda.</p>
        </div>
      </header>
      ...
      <div className="flex-1 space-y-4">
        {filteredAppointments.length > 0 ? (
          filteredAppointments.map((apt: any) => (
            <Link 
              key={apt.id} 
              href={`/nurse/appointments/${apt.id}`}
              ...
              <div className="flex justify-between items-start mb-3">
                <div>
                  <h3 className="font-bold text-[#1B4332] text-lg mb-1">
                    Pasien: {apt.patientName || 'N/A'}
                  </h3>
                  <p className="text-sm text-slate-600">
                    Jenis Layanan: <span className="font-medium">{apt.serviceType || 'Care'}</span>
                  </p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex gap-2 text-xs">
                  <span className="bg-[#E2F1EC] text-[#37A47C] px-2.5 py-1 rounded-lg font-medium">
                    Rp {apt.totalPrice?.toLocaleString('id-ID') || '0'}
                  </span>
                  {apt.serviceName && (
          ...
          <div className="bg-white rounded-[2rem] p-6 border border-slate-100 flex flex-col items-center justify-center text-center py-16 h-full mt-4">
            ...
            <h3 className="font-bold text-[#1B4332] text-lg mb-2">Belum ada jadwal {activeTab}</h3>
            <p className="text-sm text-slate-500 font-light max-w-[240px]">
              {activeTab === 'aktif' ? 'Tidak ada jadwal yang ditampilkan saat ini.' : 'Tidak ada riwayat untuk ditampilkan pada kategori ini.'}
            </p>
          </div>
        )}
      </div>
